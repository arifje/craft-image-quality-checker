...
- **Create Video**: Editors can animate the current image with optional motion instructions. This queues a 720p MP4 generated by Google Gemini Omni Flash, chooses landscape or portrait output from the source orientation, and offers the result as a protected download. It never creates, relates, or replaces a Craft asset.
...
2. Enter the key under **Enhancement → Video generation → Google AI API key**. This field remains visible because it is also used by **Create Video**.
...
The control-panel **Create Video** action also uses this Google AI key, even when OpenAI or xAI is selected as the image-enhancement provider. It sends the source image and the editor's optional video instructions to `gemini-omni-1.1-flash`. The generated MP4 remains in protected temporary storage while it is available for download; choosing **Done** removes it immediately. See Google's [Gemini Omni Flash video documentation](https://ai.google.dev/gemini-api/docs/omni) for API access, supported regions, safety restrictions, and billing details.
